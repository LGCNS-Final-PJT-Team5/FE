...
const ReactionSpeedChart: React.FC<ReactionSpeedChartProps> = ({ events, height = 200, score = 100 }) => {
  const width = screenWidth - 80;
  const chartHeight = height - 40;
  
  // 이벤트가 없는 경우 기본 그래프 표시
      ...
      <View style={[styles.chartSpecialContainer, { height }]}>
        ...
        <View style={styles.scoreContainer}>
          <Text style={styles.scoreText}>
            반응 점수: <Text style={styles.scoreValue}>{score.toFixed(1)}점</Text>
          </Text>
          <Text style={styles.penaltyText}>
            느린 반응 감지: <Text style={styles.penaltyValue}>0회</Text> 
            (각 -10점)
          </Text>
        </View>
        
        <View style={styles.noDataContainer}>
          <Text style={styles.noDataText}>반응 속도 데이터가 없습니다</Text>
        </View>
      </View>
    );
  }
  ...
  const generateDataPoints = () => {
    const points = [];
    const numPoints = 100; // 데이터 포인트 수 증가
    ...
    <View style={[styles.chartSpecialContainer, { height }]}>
      ...
      <Svg width={width} height={chartHeight}>
        ...
        <SvgText 
          ...
          fontSize={11}
        >
          {penaltyCount > 0 ? "위험 구간 (1초 초과 반응)" : "모든 반응이 정상입니다"}
        </SvgText>
